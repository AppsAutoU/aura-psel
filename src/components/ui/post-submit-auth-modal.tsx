...
import { X, Mail, Lock, User, Phone, CheckCircle2, ArrowRight, Copy, Check } from 'lucide-react'
import { createClient } from '@/lib/supabase/client'

interface PostSubmitAuthModalProps {
  isOpen: boolean
  onClose: () => void
...
export function PostSubmitAuthModal({ 
  isOpen, 
  onClose, 
  onSuccess, 
  vagaTitulo,
  consultaToken,
  email
}: PostSubmitAuthModalProps) {
  const [mode, setMode] = useState<'success' | 'login' | 'signup'>('success')
  ...
  const handleCopyToken = () => {
    if (consultaToken) {
      navigator.clipboard.writeText(consultaToken)
      setCopied(true)
          ...
          <div className="p-8">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600">
                Sua candidatura para <span className="font-semibold text-violet-600">{vagaTitulo}</span> foi recebida.
              </p>
            </div>
            ...
            {consultaToken && (
              <div className="mb-6 p-4 bg-gray-50 rounded-xl">
                <p className="text-sm text-gray-700 mb-2 font-medium">
                  Código para consulta do status:
                </p>
                <div className="flex items-center gap-2">
                  <code className="flex-1 bg-white px-3 py-2 rounded-lg border border-gray-200 font-mono text-sm">
                    {consultaToken}
                  </code>
                  <button
                ...
                <p className="text-xs text-gray-500 mt-2">
                  Guarde este código para acompanhar o status da sua candidatura
                </p>
              </div>
            )}
            ...
            <div className="space-y-3">
              ...
              <p className="text-sm text-gray-600 mb-4">
                Com uma conta você pode acompanhar todas suas candidaturas em um só lugar
              </p>
              ...
              <button
                onClick={onClose}
                className="w-full p-3 text-gray-600 hover:text-gray-800 font-medium text-sm"
              >
                Continuar sem conta
              </button>
            </div>
        ...
        {mode === 'login' && (
          <div className="p-8">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-600 text-sm">Acompanhe todas suas candidaturas</p>
            </div>
