...
export default function HistoricoPage() {
  ...
  useEffect(() => {
    const fetchCandidatos = async () => {
      try {
        ...
        // Buscar todos os candidatos (filtro será aplicado no frontend)
  ...
  const getStatusInfo = (status: string) => {
    const statusMap: Record<string, { label: string; color: string; fase: string }> = {
      ...
      'entrevista_socios': { label: 'Aguardando Entrevista com Sócios', color: 'bg-indigo-100 text-indigo-800 border-indigo-200', fase: 'Entrevista com Sócios' },
      'reprovado': { label: 'Reprovado na Entrevista Técnica', color: 'bg-red-100 text-red-800 border-red-200', fase: 'Entrevista Técnica' },
      'reprovado_socios': { label: 'Reprovado na Entrevista com Sócios', color: 'bg-red-100 text-red-800 border-red-200', fase: 'Entrevista com Sócios' },
  ...
  // Filtrar apenas candidatos finalizados (reprovados ou contratados)
  const candidatosFinalizados = candidatos.filter(candidato => {
  ...
  const filteredCandidatos = candidatosFinalizados.filter(candidato => {
  ...
  return (
    <AdminLayout>
      <div className="p-6">
        <div className="mb-8">
          <h2 className="text-3xl font-bold text-gray-900 mb-2">Histórico de Candidatos</h2>
        ...
        <div className="mb-6 flex gap-3">
          ...
          <Button
            ...
            variant={filter === 'reprovados' ? 'default' : 'outline'}
          >
